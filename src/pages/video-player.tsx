...
export default function VideoPlayer() {
  ...
  const fetchWordInfo = async (words: SelectedWord[]) => {
    ...
    const prompt =
      wordTexts.length > 1
        ? `解释短语"${text}"在语境中的含义。请使用Markdown格式回复，不要在译文中重复短语本身：

**译文：** [直接提供中文含义，不要包含英文短语]
**用法：** 解释此短语/俚语常见使用场景
**英文例句：** 提供1个不同的简短例句
**中文翻译：** 例句的中文翻译`
        ...
        <div className="flex-1 flex flex-col">
          ...
          >
            ...
            <div className="absolute inset-0 bg-black bg-opacity-0 hover:bg-opacity-30 transition-all duration-300 group">
              ...
              {/* Top Controls */}
              <div className="absolute top-4 left-4 opacity-70 group-hover:opacity-100 transition-opacity z-20">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setLocation("/");
